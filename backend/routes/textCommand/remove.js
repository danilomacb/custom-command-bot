...
async function remove(req, res) {
  ...
  try {
    ...
    successHandler(
      res,
      200,
      `Text command removed, textCommandId: ${textCommandId}, memberUsername: ${member.discordUsername}, memberDiscriminator: ${member.discordDiscriminator}, memberId, ${member.discordUserId}, discordServerName: ${discordServer.discordServerName}, discordServerId: ${discordServer.discordServerId}`
    );
  } catch (err) {
    errorHandler(
      res,
      500,
      `Error on remove text command, save failed, textCommandId: ${textCommandId}, memberUsername: ${member.discordUsername}, memberDiscriminator: ${member.discordDiscriminator}, memberId, ${member.discordUserId}, discordServerName: ${discordServer.discordServerName}, discordServerId: ${discordServer.discordServerId}`,
      err
    );
  }
}
